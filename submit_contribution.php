...
$existquery=mysqli_query($conn,"SELECT * FROM EKA_WORD_TABLE WHERE C_ENGLISH = '$input_word' AND C_EKALIPI = '$ekalipi_word' AND (C_COUNTRY = ' ' OR C_COUNTRY = 'IN' OR C_COUNTRY ='WW' OR C_COUNTRY ='UK' OR C_COUNTRY ='US' OR C_COUNTRY ='JP')");   

if(mysqli_num_rows($existquery) > 0){
    $eresponse = "*Word already exists !";
...
if (isset($_FILES['recording']))
{
    ...
    if (!file_exists($uploadDir))
    {
        ...
        if (!mkdir($uploadDir, 0777, true))
        {
            echo json_encode(array(
                'status' => 400,
                'message' => 'Failed to create recordings folder therefor cannot save file'
            ));
            exit();
        }
    }
    if (!is_writable($uploadDir))
    {
        echo json_encode(array(
            'status' => 400,
            'message' => 'recordings folder is not writable.'
        ));
        exit();
    }
...
$ekalipi_word = htmlspecialchars($_POST['ekalipi_word'], ENT_NOQUOTES, "UTF-8"); #htmlspecialchars($_POST['ekalipi_word'], ENT_QUOTES);
$country_code = $_POST['country_code']; #htmlspecialchars($_POST['country_code'], ENT_QUOTES);
$emailId = $_POST['email_id']; #htmlspecialchars($_POST['email_id'], ENT_QUOTES);
if (!$country_code)
{
    $country_code = 'WW';
}
...
$timestamp = date("Y-m-d_H:i", $t = time());
$insert_record_sql = "INSERT INTO EKA_WORD_TABLE(C_ENGLISH,C_EKALIPI,C_COUNTRY, filename, foldername,C_OTHER,C_TIME,email) VALUES ('" . $input_word . "','" . $ekalipi_word . "','" . $country_code . "','" . $fileName . "','" . $uploadDir . "','', '" . $timestamp . "','".$emailId."')";
